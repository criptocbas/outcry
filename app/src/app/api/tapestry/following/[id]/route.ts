...
export async function GET(
  ...
  try {
    const res = await fetch(
      `${TAPESTRY_BASE}/profiles/${encodeURIComponent(id)}/following?apiKey=${API_KEY}&page=1&pageSize=${encodeURIComponent(limit)}&offset=${encodeURIComponent(offset)}`
    );
    ...
    console.error("Tapestry following fetch error:", error);
    return NextResponse.json(
